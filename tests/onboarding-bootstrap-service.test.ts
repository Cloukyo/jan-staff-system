...
const bootstrap = {
  ...
  security: {
    emailVerified: true,
    assuranceLevel: "aal2",
    legalAcceptancesCurrent: true,
  },
  steps: [
    {
      stepKey: "owner_security",
      status: "complete",
      revision: "0",
      draftPayload: {},
      validationSummary: [],
  ...
  legalDocuments: [
    {
      ...
      summary: "Commercial platform terms.",
      effectiveAt: "2026-08-01T00:00:00+00:00",
      accepted: true,
    },
    {
      ...
      summary: "Commercial privacy information.",
      effectiveAt: "2026-08-01T00:00:00+00:00",
      accepted: true,
    },
    {
      ...
      summary: "Commercial processor terms.",
      effectiveAt: "2026-08-01T00:00:00+00:00",
      accepted: true,
    },
  ],
...
describe("onboarding bootstrap service", () => {
  ...
  it("accepts a matching authoritative command response and rejects stale or malformed authority", async () => {
    const response = {
      ...
      bootstrap: {
        ...bootstrap,
        session: { ...bootstrap.session, revision: "3" },
      },
    };
    const rpc = vi.fn().mockResolvedValue({ data: response, error: null });
    await expect(
      executeOnboardingBootstrapCommand(command, { rpc }),
    ).resolves.toEqual(
  ...
  it("accepts an exact terminal replay after the authoritative workflow has advanced", async () => {
    const response = {
      commandResult: {
        ...
        resultReference: {
          organisationId: "71000000-0000-4000-8000-000000000009",
        },
        sessionRevision: "2",
        issues: [],
      },
      ...
      bootstrap: {
        ...bootstrap,
        session: { ...bootstrap.session, revision: "5" },
      },
    };
    await expect(
      executeOnboardingBootstrapCommand(command, {
      ...
      expect.objectContaining({
        commandResult: expect.objectContaining({ outcome: "replayed" }),
      }),
    );
  });
  ...
  it("accepts a strict plan-selection command and rejects client entitlement authority", async () => {
    ...
    const response = {
      commandResult: {
        ...
        outcome: "succeeded",
        dataState: "saved",
        resultCode: "trial_pending_created",
        resultReference: {
          subscriptionId: "71000000-0000-4000-8000-000000000010",
        },
        sessionRevision: "3",
        issues: [],
      },
    ...
    await expect(
      executeOnboardingBootstrapCommand(planCommand, {
        rpc: vi.fn().mockResolvedValue({ data: response, error: null }),
      }),
    ).resolves.toEqual(
      expect.objectContaining({
    ...
    await expect(
      executeOnboardingBootstrapCommand(
        {
          ...planCommand,
          payload: { ...planCommand.payload, offlineAttendance: true },
        },
        {
          rpc: vi.fn(),
        },
