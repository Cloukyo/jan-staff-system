...
export type OnboardingRoute = "/onboarding" | "/onboarding/legal" | "/onboarding/organisation" | "/onboarding/site" | "/onboarding/plan" | "/onboarding/staffing" | "/onboarding/managers" | "/onboarding/next";
...
export function authoritativeOnboardingRoute(snapshot: OnboardingBootstrapSnapshot): OnboardingRoute {
  if (snapshot.session.organisationId) {
    ...
    if (staffing !== "complete" && staffing !== "skipped") return "/onboarding/staffing";
    const managers = snapshot.steps.find((step) => step.stepKey === "manager_invitations")?.status;
    return managers === "complete" || managers === "skipped" ? "/onboarding/next" : "/onboarding/managers";
  }
